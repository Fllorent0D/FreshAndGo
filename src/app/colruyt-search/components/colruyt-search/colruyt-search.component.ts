...
export class ColruytSearchComponent implements OnInit, OnDestroy {
  ...
  searchProduct(ingredientName: string) {
    ...
    this.colruytService.searchProduct(ingredientName)
      .subscribe((list) => {
  ...
  addToBasket(basketItem: ColruytAddToBasketItem) {
    this.store
      .dispatch(new ColruytAddToBasket(basketItem.item.id, basketItem.quantity.unit, basketItem.quantity.quantity))
      .subscribe(
        (response) => {
